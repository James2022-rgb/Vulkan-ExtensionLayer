...
struct SubpassDesc final {
    ...
    friend bool operator==(SubpassDesc const& lhs, SubpassDesc const& rhs) {
        return lhs.input_attachments == rhs.input_attachments &&
    ...
    struct Hasher final {
        size_t operator()(SubpassDesc const& v) const {
            ...
            APPEND_HASH_ARRAY(v.color_resolve_attachments.data(), v.color_resolve_attachments.size(), v.color_resolve_attachments.size());
...
struct RenderPassDesc final {
    ...
    friend bool operator==(RenderPassDesc const& lhs, RenderPassDesc const& rhs) {
        return lhs.attachment_signatures == rhs.attachment_signatures &&
               lhs.subpass_descs == rhs.subpass_descs;
    }
    ...
    struct PipelineEqualTo final {
        bool operator()(RenderPassDesc const& lhs, RenderPassDesc const& rhs) const {
            bool attachments_equal = std::equal(
                lhs.attachment_signatures.begin(), lhs.attachment_signatures.end(),
...
struct FramebufferDesc final {
    ...
    friend bool operator==(FramebufferDesc const& lhs, FramebufferDesc const& rhs) {
        return lhs.render_pass == rhs.render_pass &&
               lhs.render_area.offset.x == rhs.render_area.offset.x &&
               lhs.render_area.offset.y == rhs.render_area.offset.y &&
               lhs.render_area.extent.width == rhs.render_area.extent.width &&
               lhs.render_area.extent.height == rhs.render_area.extent.height &&
...
public:
    VkRenderPass FindOrAddForRenderPassInstance(RenderPassDesc const& desc,
    ...
    VkRenderPass FindOrAddForPipeline(RenderPassDesc const& desc,
                                      PFN_vkCreateRenderPass create_render_pass,
                                      VkDevice device) {
        std::lock_guard lock(mutex_);
...
private:
    ...
    VkRenderPass FindOrAdd(RenderPassDesc const& desc,
                                  PFN_vkCreateRenderPass create_render_pass,
                                  VkDevice device) {
    ...
    static VkRenderPass CreateVkRenderPass(RenderPassDesc const& desc,
                                           PFN_vkCreateRenderPass create_render_pass,
                                           VkDevice device) {
        // TODO: If we support depth/stencil resolve, we will be relying on `VK_KHR_depth_stencil_resolve` and hence `VK_KHR_create_renderpass2`.
        ...
        for (SubpassDesc const& subpass_desc : desc.subpass_descs) {
            ...
            dst.pResolveAttachments = storage.color_attachment_refs.empty() ? nullptr : storage.color_resolve_attachment_refs.data();
    ...
    using OuterPool = std::unordered_map<RenderPassDesc, InnerPool, RenderPassDesc::PipelineHasher, RenderPassDesc::PipelineEqualTo>;
...
public:
    VkFramebuffer FindOrAdd(FramebufferDesc const& desc,
...
DeviceFeatures::DeviceFeatures(uint32_t api_version, const VkDeviceCreateInfo* create_info)
    ...
    this->dynamic_rendering = dynamic_rendering_feature_requested && (dynamic_rendering_extension_requested || VK_MAKE_VERSION(1, 3, 0) <= api_version);
}
...
VKAPI_ATTR VkResult VKAPI_CALL CreateDevice(VkPhysicalDevice physicalDevice, const VkDeviceCreateInfo* pCreateInfo,
    ...
    try {
        bool enable_layer = (features.dynamic_rendering && (!pdd->lower_has_dynamic_rendering || instance_data->layer_settings.force_enable));
        ...
        if (enable_layer) {
            ...
            for (auto chain = reinterpret_cast<VkBaseInStructure*>(const_cast<void*>(create_info.pNext)); chain; chain = const_cast<VkBaseInStructure*>(chain->pNext)) {
                switch (chain->sType) {
                    case VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES: {
...
VKAPI_ATTR VkResult VKAPI_CALL CreateGraphicsPipelines(VkDevice device,
    ...
    for (uint32_t create_info_index = 0; create_info_index < createInfoCount; ++create_info_index) {
        ...
        if (rendering_create_info == nullptr) {
            VkResult result = device_data->vtable.CreateGraphicsPipelines(device, pipelineCache, 1, &create_info, pAllocator, &pPipelines[create_info_index]);
            if (result != VK_SUCCESS) {
        ...
        bool uses_depth_stencil_attachment = false;
        {
            ...
            VkSampleCountFlagBits samples =
                create_info.pMultisampleState != nullptr ? create_info.pMultisampleState->rasterizationSamples : VK_SAMPLE_COUNT_1_BIT;
            ...
            if (rendering_create_info->depthAttachmentFormat != VK_FORMAT_UNDEFINED || rendering_create_info->stencilAttachmentFormat != VK_FORMAT_UNDEFINED) {
                VkFormat format = rendering_create_info->depthAttachmentFormat != VK_FORMAT_UNDEFINED ? rendering_create_info->depthAttachmentFormat : rendering_create_info->stencilAttachmentFormat;
        ...
        VkRenderPass render_pass = device_data->render_pass_pool.FindOrAddForPipeline(
            render_pass_desc,
            device_data->vtable.CreateRenderPass,
            device_data->device
        );

        vku::safe_VkGraphicsPipelineCreateInfo modified_create_info(&create_info, uses_color_attachment, uses_depth_stencil_attachment);
        ...
        VkResult result = device_data->vtable.CreateGraphicsPipelines(device, pipelineCache, 1, modified_create_info.ptr(), pAllocator, &pPipelines[create_info_index]);
        if (result != VK_SUCCESS) {
            pPipelines[create_info_index] = VK_NULL_HANDLE;
            collective_result = result;
        }
...
VKAPI_ATTR void VKAPI_CALL CmdBeginRendering(VkCommandBuffer commandBuffer, const VkRenderingInfoKHR* pRenderingInfo) {
    ...
    // TODO: These are our current assumptions.
    {
        ASSERT((pRenderingInfo->flags & (VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT_KHR | VK_RENDERING_SUSPENDING_BIT | VK_RENDERING_RESUMING_BIT)) == 0);
    ...
    RenderPassDesc render_pass_desc;
    {
        ...
        auto SetAttachmentSignature = [](AttachmentSignature& dst, VkRenderingAttachmentInfoKHR const& attachment_info, DeviceData const* device_data) {
        ...
        if (pRenderingInfo->pDepthAttachment != nullptr || pRenderingInfo->pStencilAttachment != nullptr) {
            ...
            // > If neither pDepthAttachment or pStencilAttachment are NULL and the imageView member of either structure is not VK_NULL_HANDLE,
            // > the imageView member of each structure must be the same
            VkRenderingAttachmentInfoKHR const& attachment_info = pRenderingInfo->pDepthAttachment != nullptr ? *pRenderingInfo->pDepthAttachment : *pRenderingInfo->pStencilAttachment;
    ...
    VkFramebuffer framebuffer = device_data->framebuffer_pool.FindOrAdd(framebuffer_desc, device_data->vtable.CreateFramebuffer, device_data->device);
